...
/// `[start_at, end_at)` overlaps the report window. Applicability rules:
/// - direct match by `target_id`, or
/// - tag match (target has the tag the maintenance specifies), or
/// - global (both `target_id` and `target_tag` are NULL)
pub async fn list_in_window(
    ...
    let tag_pattern = format!("%{}%", target_tags.unwrap_or(""));
    let results = db
        .prepare(
            "SELECT * FROM maintenance_windows
             WHERE start_at < ?3
               AND end_at > ?2
               AND (target_id = ?1
                    OR (target_tag IS NOT NULL AND ?4 LIKE '%' || target_tag || '%')
