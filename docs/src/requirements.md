...
| **G-35** | PRQ-15 | `.github/workflows/release.yml:50` publishes the release with `gh release create … --generate-notes`, which is GitHub's automatic commit and pull-request summary. `rfcs/handoffs/RELEASE.md` § Release notes requires the notes to be drawn from the dated `CHANGELOG.md` entry and to state breaking changes, operator migration steps, known issues carried forward, and what a rollback reinstates. A commit list states none of those. A release whose changelog entry was never written publishes an auto-generated body and **succeeds**. | **High at 0.28.2, not before.** Harmless for 0.28.1, which changed nothing an operator sees. It would not have been harmless for 0.28.0, whose notes needed to say *"a published credential is now refused unconditionally; any deployment still holding the old `GATEWAY_SHARED_TOKEN` will be refused on every request."* 0.28.2's changelog already carries an operator action (`wrangler d1 migrations apply`) and the fact that no audit row was lost; `--generate-notes` discards both. The curated changelog exists and is written — it simply is not what gets published. |
