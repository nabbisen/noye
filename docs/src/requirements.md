...
| FR-AUTH-02 | The system MUST work with any standards-conformant OIDC provider, with no provider-specific code paths. | Google, Entra ID, Auth0, Okta, Keycloak, GitLab, and the bundled dev stub all authenticate using only configuration. A provider without a discovery document works when `OIDC_AUTH_URL`/`OIDC_TOKEN_URL`/`OIDC_JWKS_URL` are all set (FR-AUTH-03). | Implemented |
| FR-AUTH-03 | Endpoint discovery MUST be supported, with per-endpoint overrides available for providers lacking discovery. | `OIDC_AUTH_URL` / `OIDC_TOKEN_URL` / `OIDC_JWKS_URL` override discovery when set; whichever are unset still come from discovery. | Implemented |
...
| FR-RBAC-07 | The identity mapping MUST key on the OIDC `sub` claim, not on email. | Changing a user's email at the IdP does not create a second account. | Implemented |
...
| FR-TGT-08 | A target MUST record who created and who last updated it. | Both attributes are readable through the API and preserved across export/import. | Implemented |
...
| FR-TGT-10 | Tags MUST match exactly when used for scoping; a tag MUST NOT match by substring. | A window scoped to `api` does not apply to a target tagged `api-v2`. | Implemented |
...
| FR-INC-03 | At most one incident per target MUST be open at any time. | Enforced by the database, not only by application flow. | Implemented |
...
| FR-INC-08 | Every resolved incident MUST record its duration so that mean-time-to-recovery can be computed. | Duration is present for both automatic and manual resolutions. | Implemented |
...
| FR-INC-10 | The incident state set MUST contain only states the system actually produces. | No unreachable state is accepted by the schema. | Implemented |
...
| FR-SUP-03 | A suppression window MUST NOT specify both a target scope and a tag scope. A window is scoped to a target, to a tag, or to the whole deployment — never to more than one of the three. | The database rejects a window carrying both `target_id` and `target_tag`. | Implemented |
...
| FR-SUP-07 | A window whose suppression flag is disabled MUST NOT suppress notifications. | A window with `suppress_notify = false` records but does not suppress. | Implemented |
| FR-SUP-08 | An inactive window MUST NOT affect SLA calculation. | Deactivated windows are excluded from SLA queries. | Implemented |
| FR-SUP-09 | A window MUST record its creator and last updater. | Both persisted for audit. | Implemented |
| FR-SUP-10 | A window MUST NOT be saveable with an end time at or before its start time. | Rejected at both application and database level. | Partial — rejected at the database level (`CHECK (start_at < end_at)`, subject 18); no application-level pre-check exists. *(Corrected 2026-08-15: the prior status claimed the reverse — "validated in the application, not constrained in the schema" — but no such application-level check was ever found in `db/maintenance.rs`, `api/maintenance.rs`, or the gateway's form handler. That claim was already wrong before this subject; the schema constraint closing G-13's share of it is real.)* |
...
| FR-SUP-13 | A suppression window MUST state, independently, whether it silences notifications and whether it excludes its time from SLA calculation. The interface MUST present this as named situations (for example "planned maintenance", "known external outage", "expected noise"), not as two unexplained checkboxes. | Creating a window offers a small set of named situations, each stating its own consequence; the underlying record carries two independent flags. | Implemented |
...
| FR-SLA-05 | Suppression-window time MUST be excluded from the SLA denominator. | See formula below. | Implemented |
...
| FR-SLA-09 | When a reporting window is entirely excluded from SLA calculation, SLA uptime MUST be reported as not applicable, never as 100%. | The figure renders as an em dash, the same convention already used when `mttr_seconds` has no resolved incident to measure. | Implemented |
...
is "maintenance time did not happen for SLA purposes". The implementation
now performs the latter — closed by subject 13, §11 G-12.
...
| FR-USR-05 | Email addresses MUST be treated case-insensitively for identity purposes. | Two casings of one address cannot become two accounts. | Implemented |
...
| FR-MIG-08 | An import MUST produce a system state equivalent to having created the same objects through the normal path. | Imported targets are immediately monitorable. | Implemented |
| FR-MIG-09 | Import MUST be recorded in the audit trail. | One audit row per applied import. | Implemented |
| FR-MIG-10 | Import MUST resolve every cross-reference carried by the document against the target deployment **before** any write, and MUST report all unresolvable references together. | A document referencing users absent from the deployment is rejected with a count of every affected object, in dry run, having written nothing. Unresolvable references MUST NOT be silently remapped. | Implemented |
| FR-MIG-11 | Applying an import MUST NOT delete monitoring history, incidents, or channel attachments belonging to objects it updates. | A `replace` import onto an existing target leaves its check results, incidents and attachments intact. | Implemented |
...
| ID | Requirement | Acceptance criteria | Status |
|---|---|---|---|
| DR-ENT-01 | Every target MUST have exactly one state row, created with the target. | No target exists without a state row, by any creation path. | Implemented |
| DR-ENT-02 | Deleting a target MUST cascade to its dependent rows. | Foreign keys declare cascade behaviour. | Implemented |
| DR-ENT-03 | Tag scoping MUST be expressible as an exact-match relation. | A normalized tag relation, or exact-match JSON evaluation. | Implemented |
| DR-ENT-04 | Consecutive-count thresholds MUST be treated as target configuration and MUST survive export/import. | Thresholds reproduced after a round trip. | Implemented |
...
| ID | Requirement | Acceptance criteria | Status |
|---|---|---|---|
| DR-INT-01 | Boolean columns MUST be constrained to 0 or 1. | `CHECK (col IN (0,1))` present. | Implemented |
| DR-INT-02 | Numeric columns MUST be range-constrained: port 1–65535; expected status 100–599; timeout 1–300 s; retries 0–10; interval 1–1440 min; TLS threshold ≥ 0. | Corresponding `CHECK` constraints present. | Implemented |
| DR-INT-03 | Interval columns MUST enforce start < end. | `CHECK (start_at < end_at)`. | Implemented |
...
| DR-INT-05 | At most one open incident per target MUST be enforced by the database. | Partial unique index on open incidents. | Implemented |
| DR-INT-06 | A suppression window MUST NOT specify both a target scope and a tag scope. | Constraint or explicit precedence logic. | Implemented |
| DR-INT-07 | Timestamps MUST use one format across schema defaults and application writes. | RFC 3339 `YYYY-MM-DDTHH:MM:SSZ` everywhere. | Implemented |
| DR-INT-08 | Indexes MUST exist for every access path used by a list or join in normal operation. | Owner lookups, channel-to-target joins, incident ordering, window overlap, audit filtering. | Implemented |
...
| NFR-PERF-04 | Data access paths used in normal operation MUST be index-supported. | See DR-INT-08. | Implemented |
...
| NFR-REL-03 | Configuration and history MUST be recoverable after loss of the primary database. | Export plus archive restore procedure documented. | **Partial** — configuration import (§11 G-05, G-06, G-22) is fixed as of subjects 08-10; the retention-archive half of the restore path (§11 G-20) is a separate, already-closed gap not reverified as part of this work |
...
| NFR-QA-09 | Requirements marked `Not met` in this document SHOULD each acquire a regression test as they are closed. | A test exists that would fail against the pre-fix behaviour. | Implemented — **binding as a merge condition since v0.28.0, and enforced on every subject since.** *(Corrected 2026-08-15: this row still read "no such tests exist yet" long after the practice became universal — it was written before v0.28.0 and never revisited.)* Standing rule 1 in `rfcs/handoffs/README.md` makes a proven-red regression test a merge condition for every closed gap, and every gap struck in §11 names one. Proven red before the fix landed, not merely written afterwards: T-01a, T-24, T-29, T-29a (migration gate); T-76, T-83–T-85, T-87, T-88, T-90 (before/after-migration fixtures); T-78a, T-189, T-219, T-227 (revert-and-restore); and behaviour-gate assertions (e)–(h). Where an assertion could pass without the fix, the proof is required individually; where it cannot — an exact-value pin, or a scan for a string the fix deletes — it is not. |
...
| ~~G-39~~ | NFR-MNT-* | ~~`db/migration.rs` binds every `i64`/`Option<i64>` through an explicit `as i32` cast (13 sites, e.g. `Some(p) => JsValue::from(p as i32)` at :283). This is **safe from G-38's `D1_TYPE_ERROR`** — `i32` becomes a JS Number — but it is a silently truncating conversion~~, which subject 07c's own guidance forbids introducing. Found and reported, not fixed, by the dev team during 07c's sweep; **the entry's own count was wrong** — corrected during the fix to **6 genuine `i64`/`Option<i64>` sites** (`port`, `expected_status`, `tls_threshold_days`, `timeout_sec`, `retry_count`, `interval_minutes`). The other 7 of the 13 `as i32` casts are `bool` fields (`is_disabled`, `is_enabled`, `suppress_notify`, `is_active`, `on_down`, `on_up`) cast to `i32` — the same safe, unrelated pattern used identically and unflagged elsewhere in the codebase — and were deliberately left untouched. | ~~**Low.** No live failure: nothing plausibly written to `port`, `expected_status`, `timeout_sec`, `retry_count`, `interval_minutes` or `tls_threshold_days` approaches `i32`'s range. It is a latent-truncation and consistency defect~~ — **Closed 2026-08-11 — Subject 07d Step 3.** Fixed with `i64_to_d1`/`opt_i64_to_d1`, the same helpers G-38 introduced, applied to the 6 genuine sites; the codebase now converts every integer bound to D1 by one rule instead of two, confirmed by `docs/src/d1-type-boundary.md`'s boundary audit. |
...
| ~~G-42~~ | FR-AUTH-01, NFR-SEC-* | ~~`crypto::sha256()` can never succeed.~~ `crates/gateway/src/auth/crypto/digest.rs:30` ~~annotates `subtle.digest()`'s resolved value as `Uint8Array` and calls `dyn_into()` on it. `subtle.digest()` resolves to an **`ArrayBuffer`**, and an `ArrayBuffer` is never `instanceof Uint8Array` — the cast cannot succeed in any conforming JS engine.~~ The comment on the old line 34 described the correct fix and was unreachable because the annotation above it was wrong. **One call site: `auth/oidc.rs:164`, the PKCE S256 code challenge, on every login initiation.** Classified by the dev team during subject 07d Step 6 via a temporary diagnostic confirming the constructor name is `ArrayBuffer`; **re-confirmed identically under `workerd`** (`wrangler dev --local`, the actual production runtime — not only Node) during subject 07e's Step 0, before any fix was applied. | ~~**High — and it fails *closed*, so it is an outage, not a vulnerability.**~~ `sha256()` returned `Result` and `oidc.rs:166` mapped it to an error, so a login attempt failed with a 500 — it never produced a weak challenge, a predictable verifier, or a bypass; nothing was ever less secure than intended. **Confirmed on workerd: OIDC login had never worked in any deployment of this project** — the same shape as G-36 and G-38, unexecuted code hiding a defect nothing ran. **Closed 2026-08-11 — Subject 07e.** Fixed by casting to `js_sys::ArrayBuffer` first, then wrapping in `Uint8Array::new(&array_buffer)` — exactly what the original code's own comment already described; only the intermediate type annotation was wrong. `sha256("abc")` now returns the FIPS 180-4 vector under both Node and workerd; `/auth/login` produces a genuine PKCE `code_challenge` end to end (T-215) instead of a 500. All 13 `noye-gateway` WASM tests pass (T-217) and the crate is now in CI's `wasm-tests` job, proven to go red on a broken crypto test before merging (T-216) — finishing what G-40 started. |
| ~~G-43~~ | DR-LIF-05, NFR-REL-* | ~~**Retention's hourly trigger reads the wall clock, not the scheduled event.**~~ `crates/core/src/monitor/engine.rs:87` ~~gates `run_cleanup` behind `chrono::Utc::now().format("%M") == "00"`, where `now` is read at handler start. The cron is `* * * * *` — every minute — and `crates/core/src/lib.rs:96` receives the invocation as **`_event`**, discarding it.~~ `worker`'s `ScheduledEvent` exposes `schedule()` (the nominal scheduled time) and `cron()`; the handler asked the wall clock instead. Found by the dev team during subject 07f, as the reason DR-LIF-06 could not be asserted in CI. | ~~**Medium.** Cloudflare cron triggers are best-effort, not exact: an invocation that starts even slightly late reads a minute other than `00`, the branch does not fire, and retention silently skips that hour~~ — with no log line either way, because the condition simply evaluated false. The same is true of a retried invocation. This is the project's signature defect shape for the fourth time (G-21, G-32, G-33): a control that appears configured, may never execute, and says nothing when it doesn't. **Closed 2026-08-13 — Subject 07g.** The retention gate now decides from `event.schedule()`, extracted into a pure `retention_trigger` function so the decision (T-223/T-224) is host-tested without a Worker runtime. Both non-firing paths are now logged (T-227): a normal skip names the minute, an unparseable schedule is a distinct error. **DR-LIF-06 itself is still not assertable on demand**: `wrangler dev --local`'s scheduled-event simulation does not propagate a controllable nominal time through `event.schedule()` — confirmed against real `workerd` that it always returns the wall clock, traced to workerd's local-mode simulation rather than this project's code or the `worker` crate (`.git-exclude/reviewed/058-subject-07g-escalation-ruling.md`). Recorded honestly rather than papered over: `scripts/check-d1-behaviour.sh`'s (d)/T-227 asserts the *silence* half of this fix (exactly one of "ran" or "skipped, naming the minute" is always observable — never neither), not DR-LIF-06's timing claim, which stays in `scripts/deployment-verify/` pending confirmation against real Cloudflare infrastructure. |
...
| **G-44** | FR-SUP-07, FR-SUP-08, NFR-A11Y-10 | **A suppression window cannot be cancelled, edited, or deactivated once created.** `is_active` is written as a literal `1` in the `INSERT` (`crates/core/src/db/maintenance.rs:17`) and nothing ever updates it: there is no update or delete function in `db/maintenance.rs` (`create`, `list_active`, `list_in_window`, `is_under_maintenance`, `get_by_id`), none in `api/maintenance.rs` (`list`, `create`), and no route in the gateway beyond `GET /maintenance`, `POST /maintenance`, `POST /api/maintenance`. `crates/gateway/src/ui/maintenance.rs:37` compounds it by documenting `is_active` as *"computed at fetch time on Core based on the current time vs. start/end"*, a recomputation that does not exist. Disclosed by the dev team in subject 11's report (item 10) and traced further here. | **Medium.** Not suppress-forever — both consumers do bound on time (`start_at <= now AND end_at >= now`, overlap for `list_in_window`), so a window does expire at its `end_at`. But an admin who schedules a window with the wrong scope or the wrong end date has no remedy in the product and must wait it out. **Subject 11 aggravated this**: creating a window used to require a JSON API call and now takes a form on a page — the right change, and it makes the easy-to-reach action the one with no undo. The `is_active` column exists precisely to be the off switch, and nothing writes it. |
...
| ~~G-05~~ | FR-TGT-08, FR-MIG-08 | ~~The target table requires creator and updater columns, but the shared target model omits them and the import path does not populate them.~~ | ~~Configuration import into an empty database is expected to **fail a not-null constraint**.~~ **Closed — subject 08.** `noye_shared::Target` gained `created_by`/`updated_by`; on import both are always the *importing* caller, never a value carried in the document (which would name a user ID from another deployment) — confirmed against real local D1: a fresh-target import that previously failed `NOT NULL constraint failed: targets.created_by` now succeeds, and `created_by`/`updated_by` land as the caller's ID. |
| ~~G-06~~ | DR-ENT-01, DR-ENT-04, FR-MIG-08 | ~~Import does not create the per-target state row, and thresholds live on that row rather than on the target.~~ | ~~Imported targets are **not monitorable**: state lookup fails, and the configured thresholds are lost in a round trip.~~ **Closed — subject 10.** RFC 0008 (DEC-012): `success_threshold`/`failure_threshold` moved from `target_states` to `targets` (migration `0005`), so they export and import like every other decision criterion; import now creates the `target_states` row in the same operation as the target, exactly what `db::targets::create` does on the normal path. Confirmed end to end: a real monitor tick (`GET /cdn-cgi/handler/scheduled`) selected and probed an imported target, and an imported `failure_threshold = 1` produced `down` after exactly one failed check, not three. |
| ~~G-07~~ | FR-SUP-07, FR-SUP-08 | ~~The suppression check does not test the suppression flag; the SLA exclusion query tests neither the suppression flag nor the active flag.~~ | ~~A window explicitly marked as non-suppressing **still suppresses notifications**, and deactivated windows still affect SLA.~~ **Closed — subject 11.** `db/maintenance.rs`'s `is_under_maintenance` now filters `is_active = 1 AND suppress_notify = 1`; `list_in_window` now filters `is_active = 1 AND exclude_from_sla = 1` (previously neither flag was checked). `exclude_from_sla` (migration `0006` part 1) is DEC-013's second, independent flag — `MaintenanceWindow` carries it through `bool_from_d1` (G-36's own guard). Confirmed against real local D1: `scripts/check-d1-behaviour.sh`'s (e) assertion (T-52b) shows a `suppress_notify=0` window does not suppress a real scheduled-tick notification, and an `exclude_from_sla=0` window leaves a target's `excluded_seconds` at 0 while the same shape of window with the flag set moves it — a differential guard against a vacuously-passing filter. |
| ~~G-12~~ | FR-SLA-05 | ~~Suppressed time is removed from measured downtime, but the denominator remains the full window length.~~ | ~~Reported SLA does not match the stated definition or the on-screen explanation.~~ **Closed — subject 13.** `stats::compute_sla` now computes `effective_window_seconds = (window_seconds - excluded_seconds).max(0)` and reports `sla_uptime_ratio` against that denominator — `None` ("not applicable", FR-SLA-09) rather than a claimed 100% when the whole window is excluded, the same `Option`-and-em-dash convention `mttr_seconds` already used. The CSV `maintenance_seconds` column is renamed `excluded_seconds` (breaking change — see `CHANGELOG.md`). Confirmed by five new unit tests (T-67–T-71, `crates/core/src/stats.rs`) and, end to end against real local D1, `scripts/check-d1-behaviour.sh`'s (g) assertion (T-70a): a window covering an entire 24h report period reports `excluded_seconds` equal to the full window and `sla_uptime_ratio: null`. |

### Medium

| ID | Requirement | Finding | Consequence |
|---|---|---|---|
| ~~G-08~~ | FR-SUP-03, DR-INT-06 | ~~Scope is evaluated as a disjunction of target, tag, and global, with no precedence and no exclusivity constraint.~~ | ~~A window naming both a target and a tag applies more broadly than intended.~~ **Closed — subject 12.** Migration `0006` part 3 adds `CHECK (NOT (target_id IS NOT NULL AND target_tag IS NOT NULL))` to `maintenance_windows` — the ambiguous state is unrepresentable rather than resolved by precedence logic two queries would have to keep agreeing on forever (FR-SUP-03 restated to match). Pre-existing violating rows are resolved before the constraint is added: `target_tag` is cleared wherever both columns were set, target scope winning per FR-SUP-03. The gateway's new create form (subject 11) makes the ambiguous combination unreachable in the first place — scope is a radio choice, not two independently-fillable fields. |
| ~~G-09~~ | FR-TGT-10, DR-ENT-03 | ~~Tags are stored as JSON text and matched with a substring pattern.~~ | ~~A window scoped to `api` also matches `api-v2`; `prod` also matches `production`. **Silent over-suppression.**~~ **Closed — subject 12.** `targets.tags` (JSON-array TEXT) is dropped; a normalized `target_tags(target_id, tag)` relation (migration `0006` part 2, backfilled from the JSON before the column drop) replaces it. Both `db/maintenance.rs` queries now join `EXISTS (SELECT 1 FROM target_tags tt WHERE tt.target_id = ?1 AND tt.tag = target_tag)` in place of `LIKE '%' || target_tag || '%'` — exact by construction. `noye_shared::Target.tags` stays a derived `Option<String>`, computed from the relation on read (`db/targets.rs::TARGET_COLUMNS`) and written through it (`set_tags`); the configuration document's wire shape is unchanged. Confirmed against real local D1: `scripts/check-d1-behaviour.sh`'s (f) assertion (T-66b) shows a window scoped to tag `api` does not suppress a target tagged `api-v2`. |
| ~~G-10~~ | FR-INC-08 | ~~Manual resolution computes and stores duration; automatic resolution does not.~~ | ~~Automatically resolved incidents — the overwhelming majority — are **missing from mean-time-to-recovery**, making the figure misleading rather than merely incomplete.~~ **Closed — subject 14.** `db::incidents::auto_resolve` now computes `duration_sec` in SQL (`CAST(strftime('%s', ?1) AS INTEGER) - CAST(strftime('%s', opened_at) AS INTEGER)`, referencing each row's own `opened_at` since the statement updates every open incident for the target at once), not as a raw `i64` bind (G-38's shape). `stats::compute_sla`'s MTTR also derives duration from `resolved_at - opened_at` when `duration_sec` is null, so rows auto-resolved before this fix are not permanently excluded. Confirmed against real local D1 (`scripts/check-d1-behaviour.sh`, T-75a): a target driven down then up through two real scheduled ticks ends with a non-null `duration_sec` and a non-null MTTR. |
| ~~G-11~~ | FR-INC-03, DR-INT-05 | ~~Single-open-incident-per-target is a property of the application flow, with no database constraint.~~ | ~~Re-entrant scheduling, manual operations, or any future concurrency can produce duplicate open incidents.~~ **Closed — subject 15.** Migration `0007` adds `CREATE UNIQUE INDEX idx_incident_one_open ON incidents(target_id) WHERE status = 'open'` — a partial unique index, so the constraint is scoped to `open` alone (`acknowledged` is removed by a later subject and never participated). Pre-existing duplicates are resolved by the migration itself before the index is created: the earliest-opened incident per target stays open, later duplicates are force-resolved with a `resolution_note` recording why. Confirmed via `scripts/check-migrations.sh` (T-76, must-fail-first: a second open insert succeeds before `0007`, refused after; T-77: resolving the first allows a new one; T-78: the migration's own duplicate-resolution ran correctly) — the right instrument per pre-flight `.git-exclude/reviewed/063-m2c-preflight.md` §6, since `noye-core` cannot run wasm tests at all (G-37) and this needs no D1 or Wrangler. |
| ~~G-13~~ | DR-INT-01..03 | ~~Boolean, range, and interval constraints are absent from the schema.~~ | ~~Out-of-range values can enter through import, CLI, or direct database access, bypassing application validation.~~ **Closed — subject 18.** Migration `0009` adds `CHECK (col IN (0,1))` to all ten boolean columns across seven tables (`users.is_active`, `targets.is_disabled`, `check_results.is_success`, `maintenance_windows.suppress_notify`/`exclude_from_sla`/`is_active`, `notification_channels.is_enabled`, `target_notifications.on_down`/`on_up`, `retention_policies.archive_to_r2` — deliberately *not* `target_states.consecutive_successes`/`consecutive_failures`, which are counters, not booleans); range `CHECK`s on `targets.port`/`expected_status`/`timeout_sec`/`retry_count`/`interval_minutes`/`tls_threshold_days`; `success_threshold`/`failure_threshold BETWEEN 1 AND 10` (zero excluded — it would mean "transition on no evidence"); `CHECK (start_at < end_at)` on `maintenance_windows`. Confirmed against a fresh `sqlite3` database (`scripts/check-migrations.sh`, T-83–T-90, T-92): every case's own baseline proves the value was accepted before `0009` and refused after, on the same fixture — the same before/after-migration-fixture shape ruling 065 recorded as the preferred proof "where a subject's fix is a migration." |
| ~~G-14~~ | DR-INT-07 | ~~Schema defaults produce space-separated timestamps; the application writes RFC 3339.~~ | ~~Because scheduling and overlap comparisons are string comparisons, mixed formats compare **incorrectly but silently**.~~ **Closed — subject 18.** All 15 occurrences of `DEFAULT (datetime('now'))` across 9 tables replaced with `DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ','now'))`, and existing rows normalised in the same column-copy — except `audit_logs.action_time`, deliberately left byte-for-byte unchanged because it participates in the audit hash chain's canonical serialization (reformatting a historical row's `action_time` would desynchronise it from its own stored `row_hash`, corrupting tamper-evidence to fix an unrelated defect); only that column's *future*-row default changed. Confirmed T-91 (schema-default style and application style compare identically for the same instant, after disagreeing before) and, for `audit_logs` specifically, a dedicated guard proving every classification-relevant column — `action_time` included — is preserved byte-for-byte across `0009`, the same discipline T-25/T-29c already apply across `0004`. |
| ~~G-16~~ | FR-USR-05, FR-RBAC-07 | ~~The email uniqueness constraint is case-sensitive, and identity resolution leans on email.~~ | ~~Case variation from a provider can create a **duplicate account** for one person. A stable subject identifier column would remove the dependency.~~ **Closed — subject 19.** Migration `0010` rebuilds `users` with `email TEXT NOT NULL UNIQUE COLLATE NOCASE` and a new `sub TEXT UNIQUE` column (SQLite refuses `ALTER TABLE … ADD COLUMN … UNIQUE`, so the rebuild folds both changes into one pass; every existing row starts with `sub = NULL`, since SQLite treats each `NULL` as distinct under `UNIQUE`, so many unclaimed rows coexist). `db::users::resolve_by_identity` matches by `sub` first; on no match it falls back to email, but only against a row whose `sub` is still `NULL` — a row already claimed by a different subject never matches (T-98, the bypass guard) — and backfills the winning row's `sub` via `UPDATE … WHERE id = ? AND sub IS NULL`, re-querying by `sub` afterward so a concurrent first login that wins the race is picked up rather than erroring, while one that loses it is refused rather than handed the winner's identity (pre-flight §7; ruling `.git-exclude/reviewed/069-m2d-ruling.md` §1 caught the initial version returning the wrong identity here, fixed before merge). Pre-existing case-duplicate emails are refused atomically, not silently resolved (T-98c, must-fail-first against a real dup fixture, confirms zero leftover `users_new` and a diagnostic query in the migration's own header). Confirmed against real local D1 (`scripts/check-d1-behaviour.sh` block (i)): T-96 first-login backfill, T-98a re-cased email reuses the same row, T-95 changed email under a stable `sub` still resolves to the same account, T-97 unknown sub+email returns no match, T-98 a known email under an unrelated `sub` returns no match. |

### Low

| ID | Requirement | Finding | Consequence |
|---|---|---|---|
| ~~G-15~~ | DR-INT-08, NFR-PERF-04 | ~~Several access paths lack indexes, notably channel-to-target lookups, incident ordering, window overlap, and audit filtering.~~ | ~~Acceptable at present scale; degrades as history accumulates.~~ **Closed — subject 18.** Three new indexes added in migration `0009`: `idx_channels_owner` (`notification_channels.owner_id` — an FK with no supporting index), `idx_target_notifications_channel` (the reverse channel-to-target lookup; the primary key `(target_id, channel_id)` only supports the forward direction), `idx_audit_action_type` (`audit_logs.action_type` filtering). Incident ordering (`idx_incidents_target`) and window overlap (`idx_maint_active`) already existed and are confirmed still present (T-93, T-94). |
| ~~G-17~~ | FR-INC-10 | ~~The schema permits an `acknowledged` incident state that no code produces.~~ | ~~Unreachable state invites future divergence. Either implement acknowledgement with its own timestamp and actor, or remove the value.~~ **Closed — subject 17, DEC-014.** `'acknowledged'` removed from `incidents.status`'s `CHECK` (migration `0009`); RFC 0010 (the design this would have followed if implemented) withdrawn, moved to `rfcs/archive/`. Confirmed T-83 (must-fail-first: accepted before `0009`, refused after). |
...
| ~~G-19~~ | FR-AUTH-03, FR-AUTH-02 | ~~No per-endpoint OIDC override variables exist; endpoint resolution is discovery-only.~~ | ~~A provider that does not publish a discovery document is unsupported. FR-AUTH-02's "any standards-conformant provider" was overstated.~~ **Closed — subject 20.** `Config` (`auth/oidc.rs`) gains `auth_url_override`/`token_url_override`/`jwks_url_override`, populated from `OIDC_AUTH_URL`/`OIDC_TOKEN_URL`/`OIDC_JWKS_URL` (external-design.md §9.1). Two independent, host-testable predicates decide whether discovery runs at all: `auth_endpoint_needs_discovery` (T-99/T-100/T-101) gates `build_authorization_request`'s call to the new `resolve_authorization_endpoint`; `token_and_jwks_need_discovery` plus `apply_token_and_jwks_overrides` gate `handle_callback`'s call to the new `resolve_token_and_jwks` — each override is independent, and discovery is skipped entirely only when all three are set. `jwt::verify_id_token` splits into itself (fetches the JWKS) plus `verify_id_token_with_jwks` (the actual signature/claims verification), so T-102 — a token signed by an unknown key is rejected under each configuration — runs in the wasm suite against a real, independently-verified RS256 fixture without needing a live fetch. Confirmed: `crates/gateway/src/auth/oidc/tests.rs` (10 host tests, T-99/T-100/T-101) and `crates/gateway/src/auth/jwt/wasm_tests.rs` (T-102, including a must-fail-first correction after an initial fixture caught the wrong failure mode — a `kid` mismatch at JWKS lookup rather than Web Crypto signature rejection). |
...
| ~~G-22~~ | FR-MIG-08, DR-ENT-01 | ~~Configuration import uses `INSERT OR REPLACE INTO targets`, which resolves conflicts by deleting the row and therefore fires `ON DELETE CASCADE` on states, results, incidents and attachments.~~ | ~~**High.** A `replace` import silently destroys operational history while reporting success.~~ **Closed — subject 09.** All five `INSERT OR REPLACE` sites in `db/migration.rs` (targets, notification_channels, maintenance_windows, users, target_notifications — the original count found three, corrected to five during pre-flight) replaced with explicit `INSERT ... ON CONFLICT(...) DO UPDATE SET` upserts, which update in place and trigger no cascade. Confirmed against real local D1 with the critical fixture: a target carrying 2 check results, 1 open incident and 2 channel attachments, re-imported with `on_conflict = replace` — all four dependent-row counts identical before and after. |
...
| ~~G-27~~ | FR-SUP-03, FR-TGT-10 | ~~The stored tag is concatenated into the **pattern** side of `LIKE`, so a tag containing `%` or `_` acts as a wildcard. A window scoped to `%` suppresses every tagged target.~~ | ~~Medium. Compounds G-09: that is prefix collision between honest tags, this is metacharacter injection.~~ **Closed — subject 12.** The same `target_tags` exact-match relation that closed G-09 makes this structurally impossible rather than merely escaped: a stored tag is never concatenated into a `LIKE` pattern at all, since no `LIKE` remains in `db/maintenance.rs` (guarded by a source-scan test, `db/maintenance/tests.rs::no_like_remains`). Confirmed against real local D1: `scripts/check-d1-behaviour.sh`'s (f) assertion (T-66b) shows a window scoped to the literal tag `%` does not suppress a target tagged `prod`, while still correctly suppressing a target tagged exactly `%` — proving exact matching, not merely "some bytes are now escaped". |
| ~~G-28~~ | FR-INC-10 (parallel) | ~~`target_states.current_status` permits `degraded` and `maintenance`, which nothing writes, yet `db/targets.rs` counts both for the dashboard status breakdown.~~ | ~~Medium. Two of four breakdown categories are structurally always zero. Same class as G-17, on a different table, with live query code depending on it.~~ **Closed — subject 17, DEC-014.** `'degraded'`/`'maintenance'` removed from `target_states.current_status`'s `CHECK` (migration `0009`); `db/targets.rs::get_status_summary` no longer counts either; `StatusSummary` drops the two fields; the dashboard breakdown card drops the two `<dt>`/`<dd>` pairs and the two terms from its all-zero omission test (`FR-UI-08`). `BadgeKind::Degraded`/`BadgeKind::Maintenance` and their CSS/labels are **unchanged** — a live suppression window still renders `status_badge("maintenance")` (M2b's listing table) and an open incident still renders `MetricTone::Degraded` (the dashboard's own metric-card tone); neither has anything to do with `target_states.current_status`, and removing them would have broken two live features to fix an unrelated one. Confirmed T-84/T-85 (must-fail-first) and T-86a (both surviving badges/tones still render, guarding the exact collision this closure could have caused). |
| ~~G-29~~ | FR-INC-02, FR-SLA-06 | ~~`incidents.created_by` is set at open and overwritten at resolve.~~ | ~~Medium. The incident CSV's `created_by` column means "opener" for open rows and "resolver" for resolved ones.~~ **Closed — subject 16.** Split into `opened_by`/`resolved_by` (migration `0008`, `ALTER TABLE ADD COLUMN`; `created_by` left in place, unused, until a later subject rebuilds the table for unrelated CHECK-constraint reasons). Backfilled from ground truth, not from `created_by`'s current value: `open()` takes no caller and has only ever written the literal `'system'`, so `opened_by = 'system'` for every existing row regardless of status; `resolved_by` backfills from `created_by` only for rows already resolved. The incident CSV export (I-08) goes from nine columns to ten — a second breaking change to I-08 in the same unreleased version alongside M2b's SLA column rename, published as one coherent section per `CHANGELOG.md`. |
...
| ~~G-31~~ | FR-MIG-04, FR-MIG-06, FR-MIG-08 | ~~`include_users` defaults to **off**, so the default export carries no users — but `targets.owner_id` and `notification_channels.owner_id` are `NOT NULL` with a foreign key to `users(id)`. Import performs no reference validation before writing.~~ | ~~**High.** The default export cannot be imported into a fresh deployment, which is the primary stated use case for the configuration document. It fails with a raw constraint error rather than a validation report, contradicting FR-MIG-06's "all errors in one pass".~~ **Closed — subject 08.** `db::migration::find_unresolvable_owners` resolves every `owner_id` referenced by an incoming target or channel against D1 (existing rows, or users the same payload carries) before any write, in both dry run and a real import, and reports every unresolvable reference together — e.g. *"1 target(s) and 1 channel(s) reference users that do not exist in this deployment. Re-export with 'Include users' enabled, or create the users first."* Confirmed identical in `apply=true` and `apply=false`, with nothing written either way. |
...
Implemented in M2 (Phase 3), alongside gaps G-07, G-08, G-09, G-12 and
G-27, which shared the same two queries — see §11's remediation group 4,
subjects 11-13.
...
### ~~D-4 — Should incident acknowledgement exist?~~ · **CLOSED 2026-08-15**

**Resolution: removed, not implemented.** Recorded as DEC-014.
`'acknowledged'` is struck from `incidents.status`'s `CHECK` (migration
`0009`, subject 17). Incident states are Open and Resolved, matching the
glossary in §3. RFC 0010, which carried the design this would have
followed if implemented, is withdrawn and moved to `rfcs/archive/`.

**Re-evaluate when** the incident queue routinely holds more open
incidents than the team can hold in their heads, or more than one person
works the queue concurrently without talking — at that point design it
against a stated requirement, with the notification-suppression
interaction settled, rather than retrofitting a constraint value.
