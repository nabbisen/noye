...
| **Measured 2026-08-03 — the premise below is wrong** | **100 is the ceiling, not a margin below it.** Against `wrangler d1 execute --local`, a 101-placeholder statement fails at prepare with `variable number must be between ?1 and ?100`. `delete_by_ids` builds exactly `ids.len()` placeholders and `ids` is bounded by this constant, so a full batch issues a `DELETE` with **exactly 100 bound parameters — precisely at the enforced limit.** It works, and it has **zero headroom**: one additional bound parameter in that statement (a scope predicate, a tenant filter, a soft-delete guard) makes it 101 and it stops preparing. Guarded by **T-188**. **Caveat:** this measures Cloudflare's local emulator, not production — it may enforce the same number for a different reason, or a different one. The live check remains subject 36's. Found by the dev team during subject 07a's triage |
| **Why** | D1's per-statement bound-parameter ceiling could not be checked — no Wrangler or D1 environment was available during implementation. 100 was believed to be the conservative commonly-cited figure; see the measurement above, which shows it is the maximum. The two uses are deliberately coupled so one archived batch maps to exactly one `DELETE`: decoupling them (archive 1000, delete in chunks of 100) means a mid-chunk failure leaves rows archived but not deleted, and the next pass archives them again — violating DR-LIF-07 |
...
| Field | Content |
|---|---|
| **Decision** | `verify_chain` walks the audit chain by following `prev_hash → row_hash` from `GENESIS_HASH`, rather than loading rows in a sorted order and checking adjacency. `current_head_hash` derives the head from **that same walk** — the `row_hash` of the last row reached — rather than by `ORDER BY … LIMIT 1`. **Order is never reconstructed from stored columns, and there is one implementation of chain order, not two.** Closes G-30 |
| **Corrected 2026-07-31** | This decision originally specified the head as *the row whose `row_hash` appears as no other row's `prev_hash`*. **That query is wrong.** It returns two rows after an ordinary single-writer mid-chain deletion — the deleted row's predecessor and the true tail have the same shape — so a `log()` built on it would refuse every write after any deletion. Raised by the dev team as an escalation, reproduced against SQLite before it was built; see `.git-exclude/reviewed/027-subject-05-ruling-and-defect.md`. **The head must be the last *genesis-reachable* row, not the table's true latest**: after a deletion the true latest is itself unreachable, so chaining onto it would orphan every row written thereafter, whereas chaining onto the last reachable row keeps the live chain going and leaves the damage visible and bounded |
| **Fork behaviour** | A fork — two rows sharing a `prev_hash` — **does not refuse the write.** `log` continues on the branch the walk picks by its bounded `(action_time, id)` tiebreak and logs at error level; verification reports the losing branch as orphaned. A fork implies concurrent writers (excluded by DEC-004) or direct row insertion — the attacker the chain exists to detect. Refusing would let anyone able to insert one row freeze every mutation in the product: **an integrity control must not be convertible into a kill switch.** Consistent with DEC-011's surface-and-complete posture; **RFC 0007's proposal to reverse DEC-011 for atomic writes does not reverse this** |
| **Totality** | Every function reading `audit_logs` MUST terminate and produce a report for arbitrary row content — never a hang, a refusal, or a panic. The table may contain rows `log()` did not write; that is the module's reason to exist, not an edge case. Enforced by subject 05's T-23c after `walk_chain`'s first implementation was found to loop forever on a two-row cycle |
...
| **Consequence** | Verification gains a fourth class, **orphaned** — carries hashes but is unreachable from genesis (FR-AUD-05, `external-design.md` S-11). This is strictly more information: a deleted row previously made its *successors* look tampered, naming the wrong rows. **The honest cost, corrected 2026-07-31 and larger than first stated:** `current_head_hash` now loads and walks the whole table on **every audit write**, not merely a subquery — a few thousand rows transferred per mutation at the stated scale. Acceptable there; not acceptable indefinitely. Nothing in the canonical serialization, the hash format, or the schema changes |
| **Re-evaluate when** | **Subject 36 must measure the per-write cost against live D1** — this is now a named deliverable there, not a note, because the cost grew when the mechanism was corrected. First mitigation: a recursive CTE pushing the walk into D1, deliberately **not** specified here because D1's support for it has not been verified in this environment and specifying unverified mechanisms is how this subject went wrong twice. Second: an index on `prev_hash`. Neither disturbs this decision; a `seq` column would, and remains rejected |
